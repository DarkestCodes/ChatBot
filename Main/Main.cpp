...
#include "Libs\Standard\Wikipedia\Wiki.hpp"
...
int main(){
    ...
    cout << "\n*This ChatBot Does Have Some Realtime Information And Is Under Devlopment\n*Notice Your Name Will Be Used In The Chat!\n\n" << chatbot << ": Whats your name: ";    
    getline(cin, name);
    ...
    while (1){
        ...
        } else if (anwser.find("wikipedia") != string::npos or anwser.find("search") != string::npos){
            string wiki;
            send(chatbot, "Let's search some information on wikipedia!\n\nWhat do you want to search for?: ");
            getline(cin, wiki);
            bool wikierror = Search(wiki);
            if (wikierror){
                send(chatbot, "Sorry, but i didn't find the information that your requested!");
            } else {
                send(chatbot, "How can i assist you now?");
            }
        } else {
            send(chatbot, "I didn't understand, but do you wanna try'n search it on wiki? y/n");
            getline(cin, ask);
            transform(ask.begin(), ask.end(), ask.begin(),
                [](unsigned char c){ return std::tolower(c); });
            if (ask == "y"){
                bool wikierror = Search(anwser);
                if (wikierror){
                    send(chatbot, "Sorry, but i didn't find the information that your requested!");
                }
            } else {
